...
class Solution {
public:
    vector<int> get_topoSort(int &n, vector<vector<int>> &graph){
        vector<int> topoSort;
        ...
        while(!q.empty()){
            ...
            for(auto advanceCourse : graph[basicCourse]){
                ...
                if(indegree[advanceCourse]==0) q.push(advanceCourse);
            }
        }
    ...
    vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites) {
        ...
        if(topologicalSort.size()!=numCourses) topologicalSort.clear();
